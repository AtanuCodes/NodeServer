    ...
    for (let attempt = 1; attempt <= retryCount; attempt++) {
        try {
            ...
            if (response.data.result.resultCode === "200") {
                ...
                console.error("Authentication failed:", response.data.result.resultMessage);
            ...
            console.error(`Auth attempt ${attempt} error:`, error.response?.data || error.message || error.code);
...
async function getAllStockCompanies(token, retryCount = 3) {
    ...
    for (let attempt = 1; attempt <= retryCount; attempt++) {
        try {
            ...
            console.error(`Stock fetch attempt ${attempt} error:`, error.response?.data || error.message || error.code);
            if (error.code === 'ETIMEDOUT' || error.code === 'ECONNRESET' || (error.response && error.response.status === 401)) {
